...
/* MODIFIED TO SEED A HOT PHASE IF THERE IS NONE */
/*   IE IF THE PROMOTION FAILS WITH N_HOT == 0   */
      ...
      if(P[i].EnergySN > 0)
	{
	  ...
	  if(temp < All.Tcrit_Phase && SphP[i].d.Density * a3inv > All.PhysDensThresh * All.DensFrac_Phase)
		...
		SphP[i].ea.EntropyAvg / GAMMA_MINUS1 * pow(SphP[i].da.DensityAvg * a3inv, GAMMA_MINUS1);
	      ...
	      if(P[i].EnergySN > promotion_energy)	/* ENERGY HIGH ENOUGH, CHECK IF PROMOTION FAILS */
		{

#ifdef CS_ATTEMPTS
		  if(SphP[i].PromotionAttempts > 100)
		    {
		      printf("NEVER promoted %d %g %g %d %g %g %g %d\n", P[i].ID, P[i].EnergySN,
			     promotion_energy, SphP[i].HotNgbNum, SphP[i].ea.EntropyAvg, critical_entropy,
			     entropynew, SphP[i].PromotionAttempts);
		      P[i].EnergySN = 0.;	// Don't try again
		      SphP[i].PromotionAttempts = 0;	// Better Luck next time!
		  ...
		  if(!
		    ...
		    {
		      SphP[i].PromotionAttempts++;
		      printf("Non-promoted %d %g %g %d %g %g %g %d\n", P[i].ID, P[i].EnergySN,
			     promotion_energy, SphP[i].HotNgbNum, SphP[i].ea.EntropyAvg, critical_entropy,
			     entropynew, SphP[i].PromotionAttempts);
		    }
#else
	      ...
	      if(promotion_energy > 0 && P[i].EnergySN > promotion_energy	/* Regular Promotion */
		 && SphP[i].HotNgbNum > (All.DesNumNgb - All.MaxNumHotNgbDeviation)
		 && entropynew > SphP[i].ea.EntropyAvg)
		{
		  ...
		  if(get_random_number(P[i].ID) < prob)	/* ok, promote particle */
		    {
		      ...
		      SphP[i].Entropy = SphP[i].ea.EntropyAvg;	/* This is where the injection happens! */
		  ...
		  else		/*failed random number test */
		    sum_reservoir_non_promotion += P[i].EnergySN;
		}

	      if(promotion_energy > 0 && P[i].EnergySN > promotion_energy	/* SEED */
		 && SphP[i].HotNgbNum == 0 && entropynew > SphP[i].ea.EntropyAvg)
		{
		  n_selected++;
		  ...
		  if(get_random_number(P[i].ID) < prob)	/* ok, promote particle */
		    {
		      ...
		      printf
			("Seeded Part=%d DensityOld=%g DensityAvg=%g EntropyOld=%g EntropyNew=%g NHotNgbs=%d\n",
			 P[i].ID, SphP[i].DensityOld, SphP[i].da.DensityAvg, entropyold, SphP[i].Entropy,
			 SphP[i].HotNgbNum);
		      fflush(stdout);
		    }
	      ...
	      /* if neither regular nor seeding were successful, i.e. energy still > 0 */
	      if(P[i].EnergySN > 0)
		sum_reservoir_non_promotion += P[i].EnergySN;

	    }
	  else			/* Not a cold particle, but a hot particle */
	    {
