      ...
      if(P[p].Type == 0)
	{
	  ...
	  while(listp >= 0)
	      ...
	      if((task = ListExports[listp].origin) != ThisTask)
		{
		  ...
		  tmpPrimExch[off].TimeBin = P[place].TimeBin;
#ifdef VORONOI_SHAPESCHEME
		  tmpPrimExch[off].W = SphP[place].W;
#endif
#ifdef VORONOI_TIME_DEP_ART_VISC
	      tmpPrimExch[off].Alpha = SphP[place].alpha;
#endif
#if defined(VORONOI_BALSARA) || defined(VORONOI_TIME_DEP_ART_VISC)
	      tmpPrimExch[off].DivVel = SphP[place].v.DivVel;
	      tmpPrimExch[off].CurlVel = SphP[place].r.CurlVel;
#endif
		  for(j = 0; j < 3; j++)
		    {
