      ...
      if(P[i].Type == 0)
	{
	  ...
	  if(flag == 0)		/* active star formation */
		...
		tsfr / tcool * egyhot / (All.FactorSN * All.EgySpecSN - (1 - All.FactorSN) * All.EgySpecCold);
	      ...
	      egyeff +=  ( SphP[i].BPred[0] * SphP[i].BPred[0] + SphP[i].BPred[1] * SphP[i].BPred[1] + SphP[i].BPred[2] * SphP[i].BPred[2] ) / (2.*MU0) * x;
#endif
	      ...
	      if(dt > 0)
		{
		  if(P[i].TimeBin)	/* upon start-up, we need to protect against dt==0 */
		      ...
		      if(SphP[i].i.Injected_BH_Energy > 0)
			{
		      ...
		      double gauss2gadget = All.Time * All.Time * sqrt(All.UnitTime_in_s * All.UnitTime_in_s * All.UnitLength_in_cm / All.UnitMass_in_g);
		      if(All.ComovingIntegrationOn) gauss2gadget /= All.HubbleParam;

		      double mag_seed = All.SnSeedField * pow((All.SnSeedRadius / (All.Time * PPP[i].Hsml)),2);
		      double mag_norm = sqrt(P[i].Mass / SphP[i].d.Density * 3.0 / 8.0 / M_PI * pow(PPP[i].Hsml,3) * pow(All.SnSeedSoftening,3)*(1.0 + pow(All.SnSeedSoftening,3)));
		      double vabs = sqrt(SphP[i].VelPred[0]*SphP[i].VelPred[0] + SphP[i].VelPred[1]*SphP[i].VelPred[1] + SphP[i].VelPred[2]*SphP[i].VelPred[2]);

		      double magseed = mag_norm * cloudmass / P[i].Mass * All.FactorSN * dtime / tsfr * mag_seed * gauss2gadget;
		      SphP[i].MagSeed[0] = magseed * SphP[i].VelPred[0] / vabs;
		      SphP[i].MagSeed[1] = magseed * SphP[i].VelPred[1] / vabs;
		      SphP[i].MagSeed[2] = magseed * SphP[i].VelPred[2] / vabs;
#endif
