				   ...
				   aphys */ )
{
  ...
  if(P[p].Type == 0)
	...
	dt = dt_courant;
      ...
      if(SphP[p].eta2 != 0) 
      {
         dt_eta =  All.CourantFac * All.CourantFac * PPP[p].Hsml * PPP[p].Hsml / (All.cf_afac2 * SphP[p].eta2);
  ...
  if((1 << P[p].Type) & (SIDM))
    {
      if(P[p].sidm_Density>0.0 && P[p].sidm_VelDisp>0.0)
        {
          dt_sidm = SIDM_DTIME_FAC / ((All.HubbleParam * All.HubbleParam * All.cf_a3inv * P[p].sidm_Density) * sidm_cross_sigma(P[p].sidm_VelDisp / All.cf_atime)  * (P[p].sidm_VelDisp / All.cf_atime)); 
          if(dt_sidm < dt)
            dt = dt_sidm;
        }
    }
  ...
  if(!(ti_step > 0 && ti_step < TIMEBASE))
	...
	printf("hydro-frc=(%g|%g|%g) dens=%g hsml=%g  dTdE=%g\n", SphP[p].a.HydroAccel[0], SphP[p].a.HydroAccel[1],
	       SphP[p].a.HydroAccel[2], SphP[p].d.Density, PPP[p].Hsml,SphP[p].e.dDtEntropy);
