...
#define WORKSIZE 100000
  ...
  double kstart =
    1.01 * pow(10.0, PowerTableList[0].logk) / (All.InputSpectrum_UnitLength_in_cm / All.UnitLength_in_cm);
  double kend =
    0.99 * pow(10.0,
	       PowerTableList[NPowerTable -
			      1].logk) / (All.InputSpectrum_UnitLength_in_cm / All.UnitLength_in_cm);
...
static void setup_powertable_for_given_time(double a)
{
  ...
  s =
    (log10(a) - log10(TransFunc[tfunc_left].atime)) / (log10(TransFunc[tfunc_right].atime) -
						       log10(TransFunc[tfunc_left].atime));
...
void init_transfer_functions(void)
{
  ...
  if(dp != NULL)
	...
	{
	  if(strncmp
	     (ep->d_name, All.KspaceBaseNameTransferfunctions,
	      strlen(All.KspaceBaseNameTransferfunctions)) == 0)
	    {
	      ...
	      do
		{
		  ...
		  if(fscanf
		     (fd, " %lg %lg %lg %lg %lg %lg %lg", &k, &T_cdm, &T_b, &dummy, &dummy, &T_nu,
		      &T_tot) == 7)
		    NPowerTable++;
		  else
		    break;
