	      ...
	      if(All.PartAllocFactor != save_PartAllocFactor)
		{
		  ...
		  if(ThisTask == 0)
		    printf("All.TotNumPart=%llu, All.TotN_gas=%llu, All.TotN_stars=%llu \n",
			   (unsigned long long) All.TotNumPart, (unsigned long long) All.TotN_gas, (unsigned long long) All.TotN_stars);
		  if(All.TotN_stars == 0)
		    All.MaxPartMet =
		      ...
		      All.PartAllocFactor * (All.TotN_stars / NTask +
                  ...
                  if(ThisTask == 0)
                    printf("All.MaxPart=%llu, All.MaxPartSph=%llu, All.MaxPartMet=%llu \n",
                           (unsigned long long) All.MaxPart,  (unsigned long long) All.MaxPartSph, (unsigned long long) All.MaxPartMet);
