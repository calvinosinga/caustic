...
#else
            double u, uh, rho, rhoh, temp, ne, nh0h = 0;
	    double Metallicities[LT_NMet];

                                                                         
            u = DMAX(All.MinEgySpec, SphP[pindex].Entropy / GAMMA_MINUS1 * pow(SphP[pindex].d.Density * a3inv, GAMMA_MINUS1) *
                     All.UnitPressure_in_cgs / All.UnitDensity_in_cgs);
	    rho =
              SphP[pindex].d.Density * a3inv * All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam;

            set_metallicities(pindex, &Metallicities[0], a3inv);
            
            if(SphP[pindex].XColdCloud > 0)                              /* NH for the hot phase */
              {
                if(SphP[pindex].XColdCloud < 1)
                  {
                    uh = u - rho * SphP[pindex].XColdCloud * All.EgySpecCold;         
                    rhoh = rho * (1 - SphP[pindex].XColdCloud);

                    Metallicities[HydPos] *= (1 - SphP[pindex].XColdCloud);
                    temp = (MyFloat) convert_u_to_temp(uh, Redshift, DZ, &Metallicities[0]);
                    nh0h = SphP[pindex].XColdCloud + (1 - SphP[pindex].XColdCloud) * LT_AbundanceRatios(temp, rhoh, &ne);

                    Metallicities[HydPos] *= SphP[pindex].XColdCloud / (1 - SphP[pindex].XColdCloud);
                  }
                else
                  nh0h = 0;

                temp = All.TempClouds;
                rho *= SphP[pindex].XColdCloud;                
              }
            else
              temp = (MyFloat) convert_u_to_temp(u, Redshift, DZ, &Metallicities[0]);
            
            nh0 = LT_AbundanceRatios(temp, rho, &ne);

            if(SphP[pindex].XColdCloud > 0)
              nh0 = nh0 * SphP[pindex].XColdCloud + nh0h * (1 - SphP[pindex].XColdCloud);
#endif
	...
	if(P[pindex].Type == type)
	  {
	    ...
	    *fp_single++ = SphP[pindex].x;
#else
	    get_starformation_rate(pindex, &Temperature, &xclouds);
	    *fp_single++ = xclouds;
	    xclouds = 0;
#endif
	    n++;
	...
	if(P[pindex].Type == type)
	  {
            ...
            if(SphP[pindex].XColdCloud > 0)
              {
                u -= rho * SphP[pindex].XColdCloud * All.EgySpecCold;         /* obtain the energy density of the hot phase */
			...
			if(ThisTask == writeTask && All.SnapFormat == 3 && header.npart[type] > 0)
			  {
			    ...
			    hdf5_dataset =
			      ...
			      //	H5P_DEFAULT);
			      H5Dcreate1(hdf5_grp[type], buf, hdf5_datatype, hdf5_dataspace_in_file,
					H5P_DEFAULT);
...
void write_header_attributes_in_hdf5(hid_t handle)
{
  ...
  //error: too few arguments (same for all other H5Pcreate()
  ...
  hdf5_attribute = H5Acreate(handle, "SpectralIndex_CR_Pop", H5T_NATIVE_DOUBLE, hdf5_dataspace, H5P_DEFAULT, H5P_DEFAULT);
  ...
  hdf5_dataspace = H5Screate(H5S_SCALAR);
  hdf5_attribute = H5Acreate(handle, "Flag_DoublePrecision", H5T_NATIVE_INT, hdf5_dataspace, H5P_DEFAULT, H5P_DEFAULT);
  H5Awrite(hdf5_attribute, H5T_NATIVE_INT, &header.flag_doubleprecision);
  H5Aclose(hdf5_attribute);
  H5Sclose(hdf5_dataspace);

  hdf5_dataspace = H5Screate(H5S_SCALAR);
  hdf5_attribute = H5Acreate(handle, "Flag_IC_Info", H5T_NATIVE_INT, hdf5_dataspace, H5P_DEFAULT, H5P_DEFAULT);
