...
#if defined(SMOOTH_DIVB) || (defined(DIVBCLEANING_DEDNER) && !defined(RELDIVB)) || defined(SMOOTH_ROTB) || defined(BSMOOTH) || (defined(LT_STELLAREVOLUTION) && !defined(LT_DONTUSE_DENSITY_in_WEIGHT)) || defined(LT_SMOOTH_Z) || defined(LT_SMOOTH_XCLD) || defined(LT_TRACK_WINDS)

#if defined(LT_STELLAREVOLUTION) || defined(BSMOOTH_BND)
...
#if defined(LT_SMOOTH_ALLMETALS)
		  for(sp = 0; sp < LT_NMetP; sp++)
		    Zrho[sp] += (MyDouble) SphP[j].Metals[sp] * wk;
#else
...
#if (defined(DIVBCLEANING_DEDNER) || defined(SMOOTH_ROTB) || defined(BSMOOTH) || defined(SCAL_PRO_CLEAN)) || defined(LT_SMOOTH_Z) || defined(LT_SMOOTH_XCLD) || defined(LT_TRACK_WINDS)
  if(P[i].Type == 0)
#ifdef BSMOOTH_BND
   if(P[i].ID == 0)
    if(SphP[i].alfa2  == 0)
