...
void subfind(int num)
{
  ...
  limit = 0.8 * All.PartAllocFactor * All.TotNumPart / NTask;
#else
...
void subfind_save_final(int num)
{
  ...
  ID_list = mymalloc("ID_list", sizeof(struct subfind_id_list) * (int)(All.MaxPart * 1.5));
...
void sort_sub_groups()
{
  ...
  send_SubGroup = (struct subgroup_properties *) mymalloc("send_SubGroup", nexport * sizeof(struct subgroup_properties));
