...
/* DISTORTION PARTICLE SETUP */
  ...
  for(i = 0; i < N_gas; i++)	/* initialize sph_properties */
...
#ifdef STATICBRANDT_2D
	r = sqrt(P[i].Pos[0] * P[i].Pos[0]);
        m = (BRANDT_OmegaBr) * 10E-3 / sqrt(1 + (r / BRANDT_R0) * (r / BRANDT_R0));
     	
        P[i].Vel[2]=   sqrt(All.G) * m * (P[i].Pos[0]);
#else
	r = sqrt((P[i].Pos[0] - LONG_X/2.) * (P[i].Pos[0] - LONG_X/2.) + (P[i].Pos[1] - LONG_Y/2.) * (P[i].Pos[1] - LONG_Y/2.));
        m = (BRANDT_OmegaBr) * 10E-3 / sqrt(1 + (r / BRANDT_R0) * (r / BRANDT_R0));
     	
	P[i].Vel[0]= - sqrt(All.G) * m * (P[i].Pos[1] - LONG_Y/2.);
     	P[i].Vel[1]=   sqrt(All.G) * m * (P[i].Pos[0] - LONG_X/2.);
#endif
        if(P[i].Type == 0)
	  for(j = 0; j < 3; j++)
	     SphP[i].VelPred[j]=P[i].Vel[j];
	...
	P[i].Vel[0]=0.0;
	P[i].Vel[1]=0.0;
	P[i].Vel[2]= - FS_SHEAR_Q*FS_SHEAR_OMEGA*(P[i].Pos[0]);
        if(P[i].Type == 0)
	  for(j = 0; j < 3; j++)
	     SphP[i].VelPred[j]=P[i].Vel[j];
#endif
