		  ...
		  if(Left[i] > All.MaxHotHsmlParam * PPP[i].Hsml)	/* prevent us from searching too far */
		    {
		      ...
		      P[i].TimeBin = -P[i].TimeBin - 1;	/* Mark as inactive */
#ifdef CS_SEED
		      /* Ad-hoc definition of SAvg and RhoAvg in case  */
		      /* of no hot neighbours (used for seeding).      */
		      /* The target density is defined as 1/2 of the   */
		      /* density threshold, and the target entropy     */
		      /* corresponds to 2 * T_crit at this density     */

		      if(SphP[i].HotNgbNum == 0)
			{
			  SphP[i].da.DensityAvg = 0.5 * All.PhysDensThresh * All.DensFrac_Phase / a3inv;
			  ...
			  SphP[i].ea.EntropyAvg =
			    2. * All.Tcrit_Phase / pow(SphP[i].da.DensityAvg * a3inv,
						       GAMMA_MINUS1) * All.UnitMass_in_g /
			    All.UnitEnergy_in_cgs * BOLTZMANN / PROTONMASS / mu;

			  //              SphP[i].da.DensityAvg = SphP[i].d.Density / 100;
			  //              SphP[i].ea.EntropyAvg = SphP[i].Entropy * 1000;

			  printf("WARNING: ID=%d i=%d used ad-hoc RhoAvg=%g and SAvg=%g\n",
				 P[i].ID, i, SphP[i].da.DensityAvg, SphP[i].ea.EntropyAvg);
			  printf("VALUES: ID=%d xhyd=%g yhel=%g ne=%g mu=%g\n", P[i].ID, xhyd, yhel, ne, mu);
			  fflush(stdout);
#endif
			}
