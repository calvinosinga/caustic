...
void compute_global_quantities_of_system(void)
{
  ...
  struct {double val; int where;} ain,aout;
#endif
...
#if defined(MAGNETIC_STATISTICS) || defined(FS_ALFA2_QUENCH)
 	  sys.EnergyMag += P[i].Mass / SphP[i].d.Density * MU0_1 / 2.0 * (mag[0]*mag[0] + mag[1]*mag[1] + mag[2]*mag[2]);
#endif
#if defined(MAGNETIC_STATISTICS) && defined(TRACEDIVB)
	  if(P[i].ID != 0 && (SphP[i].b2.BPred[0]*SphP[i].b2.BPred[0]+SphP[i].b2.BPred[1]*SphP[i].b2.BPred[1]+SphP[i].b2.BPred[2]*SphP[i].b2.BPred[2]) != 0)
	  sys.DivBerr +=  fabs(SphP[i].divB)*PPP[i].Hsml*P[i].Mass/ 
...
#endif
#ifdef FS_ALFA2_QUENCH
	 if(fabs(SphP[i].alfa2) > sys.MaxAlfa2)  sys.MaxAlfa2 = fabs(SphP[i].alfa2);
#endif
#if defined(FS_ALFA2_TURB)
...
#if defined(MAGNETIC_STATISTICS) && !defined(AB_SHOCK_MAGNETIC)
	  double alfven_i2 = (mag[0]*mag[0] + mag[1]*mag[1] + mag[2]*mag[2]) * MU0_1 / SphP[i].d.Density;
