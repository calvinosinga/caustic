...
void cs_read_coolrate_table_WSS(void)
{
  ...
  for(rs_tab = 0; rs_tab < Nrs; rs_tab++)
      ...
      for(rho_tab = 0; rho_tab < Nrho; rho_tab++)	/* loop over density bins */
	{
	  ...
	  sprintf(table_filename, "CoolingTablesWSS08_NCC/WSS08_z=%s_nH=%s_Z=%s.txt", rsbins[rs_tab],
		  rhobins[rho_tab], metalbins[z_tab]);
#else /* Compton Cooling included in tables. Will not work well at high z */
	  sprintf(table_filename, "CoolingTablesWSS08/WSS08_z=%s_nH=%s_Z=%s.txt", rsbins[rs_tab],
		  rhobins[rho_tab], metalbins[z_tab]);
#endif
...
/*definitions from cs_metals.h above*/
  ...
  if(rs > 11.)
    rs_bin = 11;		/* photodisassociation */
  if(rs <= 11. && rs > 8.98)
    rs_bin = 10;		/* high z extrapolation */
  if(rs <= 8.98 && rs > 8.00)
    rs_bin = 9;
  if(rs <= 8.00)
    rs_bin = (int) rs;
  ...
  if(logT < 2.)			// use the lowest T bin
  ...
  else
    {
      ...
      /* Interpolation to get the correct Cooling Rate */
      if(temp_bin > 119)
	temp_bin = 119;		// use max temperature

      Lambda =
	coeff_1 * WSSLambda[rs_bin][z_bin][rho_bin][temp_bin] +
	coeff_2 * WSSLambda[rs_bin][z_bin][rho_bin][temp_bin + 1];
    }
