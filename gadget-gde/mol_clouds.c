...
double mass_function(double t, double t_born, double t_lifetime)
{
  ...
  return All.MOL_CLOUDS_m1_tilde +
    All.MOL_CLOUDS_m2_tilde * exp(-(t - t_m) * (t - t_m) /
				  ((t_lifetime / sqrt(2.0)) * (t_lifetime / sqrt(2.0))));
#endif
}
...
{
  int i;
  double mean_lifetime_local = 0.0, mean_lifetime_global = 0.0, variance_lifetime_local =
    0.0, variance_lifetime_global = 0.0;
  double total_mc_mass_local = 0.0, total_mc_mass_global = 0.0, total_star_mass_local =
    0.0, total_star_mass_global = 0.0;
  ...
  All.MOL_CLOUDS_m1_tilde =
    (KAPPA * All.MOL_CLOUDS_MassStars - All.MOL_CLOUDS_MassMCs * exp(-0.5)) / (KAPPA - exp(-0.5));
  ...
  for(i = 0; i < NumPart; i++)
    {
      if(P[i].Type != 3)
	continue;
      variance_lifetime_local +=
	(P[i].MOL_CLOUDS_LifeTime - mean_lifetime_global) * (P[i].MOL_CLOUDS_LifeTime -
							     mean_lifetime_global) / All.MOL_CLOUDS_NumMCs;
    }
  ...
  if(ThisTask == 0)
    {
      ...
      printf
	(" time = %g  total mass in MCs = %g ( fraction = %g )   total mass in stars = %g ( fraction = %g )\n",
	 All.Time, total_mc_mass_global,
	 total_mc_mass_global / (total_mc_mass_global + total_star_mass_global), total_star_mass_global,
	 total_star_mass_global / (total_mc_mass_global + total_star_mass_global));
...
int do_mol_clouds(void)
{
  ...
  int minID_local = All.MOL_CLOUDS_NumMCs + All.MOL_CLOUDS_NumStars, minID_global =
    All.MOL_CLOUDS_NumMCs + All.MOL_CLOUDS_NumStars, maxID_local = 0, maxID_global = 0;
  int converted_stars_to_MCs_local = 0, converted_stars_to_MCs_global = 0, converted_MCs_to_stars_local =
    0, converted_MCs_to_stars_global = 0;
  ...
  double tracer_pos_local[] = { 0.0, 0.0, 0.0 }, tracer_pos_global[] =
  {
  0.0, 0.0, 0.0};
  double t_born_min_local = All.TimeMax, t_born_min_global = All.TimeMax, t_born_max_local =
    All.TimeBegin, t_born_max_global = All.TimeBegin;
  ...
  if(ThisTask == 0)
    {
      printf
	(" time = %g  total mass in MCs = %g ( fraction = %g )  total mass in stars = %g ( fraction = %g )  t_born_diff = %g ( %g %g )  t_lifetime_diff = %g ( %g %g )\n",
	 All.Time, total_mc_mass_global,
	 total_mc_mass_global / (total_mc_mass_global + total_star_mass_global), total_star_mass_global,
	 total_star_mass_global / (total_mc_mass_global + total_star_mass_global),
	 ...
	 t_lifetime_max_global - t_lifetime_min_global, t_lifetime_min_global, t_lifetime_max_global);
      ...
      printf(" mctracer %g %g  %g  %g  %g %d\n", All.Time, tracer_mass_global, tracer_pos_global[0],
	     tracer_pos_global[1], tracer_pos_global[2], found_tracer_global);
    }
  ...
  for(j = 0; j < All.MOL_CLOUDS_NumMCs; j++)
    {
      ...
      ID2_List_global[j] = 0;
      lifetimes_local[j] = 0;
      lifetimes_global[j] = 0;
      dyingtimes_local[j] = 0;
      dyingtimes_global[j] = 0;
    }
  ...
  MPI_Allreduce(dyingtimes_local, dyingtimes_global, All.MOL_CLOUDS_NumMCs, MPI_DOUBLE, MPI_SUM,
		MPI_COMM_WORLD);
  ...
  if(ThisTask == 0)
	...
	lifetimes_local[j] = generate_lifetime(MCs_to_convert_global + j);
      ...
      printf(" SAMPLE OF NEW IDS: %d %d %d\n", ID1_List_local[0], ID1_List_local[All.MOL_CLOUDS_NumMCs / 2],
	     ID1_List_local[All.MOL_CLOUDS_NumMCs - 1]);
    }
  ...
  MPI_Allreduce(lifetimes_local, lifetimes_global, All.MOL_CLOUDS_NumMCs, MPI_DOUBLE, MPI_SUM,
		MPI_COMM_WORLD);
  ...
  if(ThisTask == 0)
    for(j = 0; j < All.MOL_CLOUDS_NumMCs - 1; j++)
      {
	if(((sortdata1[j].ID1 == sortdata1[j + 1].ID1) && (sortdata1[j].ID1 != 0))
	   || ((sortdata2[j].ID2 == sortdata2[j + 1].ID2) && (sortdata2[j].ID2 != 0))
	   || (sortdata1[j].ID1 > sortdata1[j + 1].ID1) || (sortdata2[j].ID2 > sortdata2[j + 1].ID2))
	  printf(" WARNING\n");
      }
  ...
  MPI_Allreduce(&converted_stars_to_MCs_local, &converted_stars_to_MCs_global, 1, MPI_INT, MPI_SUM,
		MPI_COMM_WORLD);
  MPI_Allreduce(&converted_MCs_to_stars_local, &converted_MCs_to_stars_global, 1, MPI_INT, MPI_SUM,
		MPI_COMM_WORLD);
  ...
  converted_stars_to_MCs_local = 0;
  converted_stars_to_MCs_global = 0;
  converted_MCs_to_stars_local = 0;
  converted_MCs_to_stars_global = 0;
  ...
  if(ThisTask == 0)
    {
      printf(" MOL_CLOUDS: stars_to_MCs(prop)=%d MCs_to_stars(prop)=%d\n", converted_stars_to_MCs_global,
	     converted_MCs_to_stars_global);
    }
