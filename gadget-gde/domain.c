  ...
  if(ThisTask == 0)
      ...
      if(SaveKeys == 1)
	{
	  printf("snap files using keys: memory-balance=%g, memory-balance-sph=%g",
		 ...
		 maxloadsph / (((double) sumloadsph) / NTask));
#ifdef LT_STELLAREVOLUTION
...
void domain_exchange(void)
{
  ...
  if(N_stars > All.MaxPartMet)
    {
      printf("Task %d: too many stars: %d>%d, got %d ...\n",ThisTask,N_stars, All.MaxPartMet, count_get_stars);
      printf("Task %d: have %d/%d particles and %d/%d sph particle\n",ThisTask,NumPart,All.MaxPart,N_gas,All.MaxPartSph);
      endrun(787880);
    }
...
#ifdef LT_STELLAREVOLUTION
		  ntoomanystars =
		    list_N_stars[ta] + count_toget_stars - count_togo_stars - All.MaxPartMet;
...
int domain_check_for_local_refine(int i, double countlimit, double costlimit)
{
  ...
  if (node_level < All.MaxLevelToptree) flag = 1;   /* refine */
  else return 0;                  /* don't refine */
#endif
...
#endif
{
  ...
  for(j = 0; j < 3; j++) DomainCenter[j] = 0.5 * All.BoxSize; 
