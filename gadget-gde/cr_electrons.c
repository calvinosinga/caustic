...
void compute_Dpp(size_t ipart)
{
  double n_th, T, dt, m_part, v_turb, hsml, scalefactor, Eturb, Vpart, d_mps, k_min, k_max, k_sml, k_mps;
  ...
  for(ipart = 0; ipart < N_gas; ipart++)
    {				/* Physical units */
#endif
      hsml = SphP[ipart].Hsml * All.UnitLength_in_cm * All.Time / All.HubbleParam;
      m_part = P[ipart].Mass * All.UnitMass_in_g / All.HubbleParam;
      v_turb = SphP[ipart].Vrms * All.UnitVelocity_in_cm_per_s * sqrt(All.Time);
      dt = All.TimeStep * All.UnitTime_in_s * sqrt(All.Time) / All.HubbleParam;
      ...
      d_mps = hsml / pow(P[ipart].TrueNGB, 1. / 3.);	/* Mean Part Seperation  */
      Vpart = FOURTHIRDSPI * d_mps * d_mps * d_mps;

      k_min = 1 / (TURBULENCEINJECTIONSCALE * KPC2CM);
      k_max = 1 / (TURBULENCEDAMPINGSCALE * KPC2CM);

      k_mps = 1 / (2 * d_mps);	/* Nyquist frequency of SPH */
      k_sml = 1 / (2 * hsml);	/* hsml is only half the scale */

      scalefactor = (pow(k_max, -2. / 3.) - pow(k_min, -2. / 3.))
	/ (pow(k_mps, -2. / 3.) - pow(k_sml, -2. / 3.));

      SphP[ipart].Dpp = DPP_FACTOR / I_x(ipart) / n_th / sqrt(T) * ETA_T * Eturb * scalefactor / dt / Vpart;
...
static double I_x(size_t ipart)
{
  ...
  B = sqrt(SphP[ipart].B[0] * SphP[ipart].B[0]
	   + SphP[ipart].B[1] * SphP[ipart].B[1] + SphP[ipart].B[2] * SphP[ipart].B[2]);
...
static double help_density(size_t ipart, double a, int unit)
{
  switch (unit)
    ...
    case PHYSICAL:
      return (SphP[ipart].d.Density / (a * a * a) * h * h * All.UnitMass_in_g / pow(All.UnitLength_in_cm, 3));
    default:
      endrun(10100);
      return (-1);
    }
...
static double help_pressure(size_t ipart, double a, int unit)
{
  switch (unit)
    ...
    case PHYSICAL:
      return (SphP[ipart].Pressure * GAMMA_MINUS1 * pow(a, 3 * GAMMA - 6) * h * h * All.UnitPressure_in_cgs);
    default:
      endrun(10101);
      return (-1);
    }
...
static double help_internal_energy(size_t ipart, double a, int unit)
{
  ...
  switch (unit)
    {
    case GADGET:
      return (SphP[ipart].Entropy / GAMMA_MINUS1 * pow(SphP[ipart].d.Density / (a * a * a), GAMMA_MINUS1));
    case PHYSICAL:
...
static double help_speed_of_sound(size_t ipart, double a, int unit)
{
  switch (unit)
    {
    case GADGET:
      return (sqrt(GAMMA * SphP[ipart].Entropy * sqrt(a) * pow(SphP[ipart].d.Density, GAMMA_MINUS1)));
    case PHYSICAL:
      return (sqrt(GAMMA * SphP[ipart].Entropy * sqrt(a)
		   * pow(SphP[ipart].d.Density, GAMMA_MINUS1)) * All.UnitVelocity_in_cm_per_s);
    default:
      endrun(10103);
      return (-1);
    }
