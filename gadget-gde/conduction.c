...
void conduction(void)
{
  ...
  Kappa = (MyFloat *) mymalloc("Kappa", N_gas * sizeof(MyFloat));
			...
			int *nexport, int *nsend_local)
{
  ...
  MyFloat h_i, rho;
  double dx, dy, dz;
  double h_i2, hinv_i, hinv4_i, hinv_j, hinv4_j, hinv3_i, hinv3_j;
  double dwk_i, h_j, dwk_j, dwk, wk_i , wk_j;
	  ...
	  for(n = 0; n < numngb; n++)
	      ...
	      if(r2 < h_i2 || r2 < h_j * h_j)
		{
		  ...
		  if(r > 0)
			...
			{
			  u = r * hinv_i;
			  kernel_main(u, hinv3_i, hinv4_i, &wk_i, &dwk_i, 1);
			}
		      else
			dwk_i = 0;
		      ...
		      if(r2 < h_j * h_j)
			{
			  ...
			  u = r * hinv_j;
			  kernel_main(u, hinv3_j, hinv4_j, &wk_j, &dwk_j, 1);
			}
		      else
			dwk_j = 0;
