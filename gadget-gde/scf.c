...
void SCF_init(void)
{
  ...
  sinsum =
    (MyDouble *) mymalloc("sinsum", (SCF_NMAX + 1) * (SCF_LMAX + 1) * (SCF_LMAX + 1) * sizeof(MyDouble));
  cossum =
    (MyDouble *) mymalloc("cossum", (SCF_NMAX + 1) * (SCF_LMAX + 1) * (SCF_LMAX + 1) * sizeof(MyDouble));

#ifdef SCF_HYBRID
  sinsum_all =
    (MyDouble *) mymalloc("sinsum_all", (SCF_NMAX + 1) * (SCF_LMAX + 1) * (SCF_LMAX + 1) * sizeof(MyDouble));
  cossum_all =
    (MyDouble *) mymalloc("cossum_all", (SCF_NMAX + 1) * (SCF_LMAX + 1) * (SCF_LMAX + 1) * sizeof(MyDouble));
...
#ifdef SCF_SCALEFAC
  scalefac_nlm =
    (float *) mymalloc("sinsum_all", (SCF_NMAX + 1) * (SCF_LMAX + 1) * (SCF_LMAX + 1) * sizeof(MyDouble));
  FILE *fd;
  ...
  for(n = 0; n <= SCF_NMAX; n++)
	...
	Anltilde[nl(n, l)] =
	  -pow(2.,
	       8. * l + 6.) * factrl(n) * (n + 2. * l + 1.5) * tgamma(2. * l + 1.5) * tgamma(2. * l +
											     1.5) / (4. *
												     M_PI *
												     K_nl *
												     factrl(n
		  ...
		  MyDouble * az)
{
  ...
  for(l = 0; l <= SCF_LMAX; l++)
      ...
      if(SCF_NMAX > 0)
	{
	  ...
	  for(n = 1; n <= SCF_NMAX - 1; n++)
	    {
	      ultrasp[nl(n + 1, l)] =
		((2.0 * n + (2.0 * (2. * l + 1.5))) * xi * un -
		 (1.0 * n - 1.0 + (2.0 * (2. * l + 1.5))) * unm1) * 1.0 / (n + 1.0);
	      unm1 = un;
	      un = ultrasp[nl(n + 1, l)];
	      ultrasp1[nl(n + 1, l)] =
		(((2.0 * (2. * l + 1.5)) + (n + 1) - 1.) * unm1 -
		 (n + 1) * xi * ultrasp[nl(n + 1, l)]) / ((2.0 * (2. * l + 1.5)) * (1. - xi * xi));
	    }
	}
    }
  ...
  for(l = 1; l <= SCF_LMAX; l++)
      ...
      for(m = 0; m <= l; m++)
	{
	  if(l == m)
	    ...
	    dplm[lm(l, m)] =
	      (l * costh * plm[lm(l, m)] - (l + m) * plm[lm(l - 1, m)]) / (costh * costh - 1.0);
	}
    }
  ...
  for(l = 0; l <= SCF_LMAX; l++)
      ...
      for(m = 0; m <= l; m++)
	{
      ...
      ar +=
	phinltil * (-temp1 * (l / r - (2. * l + 1.) / (1. + r)) +
		    temp2 * 4. * (2. * l + 1.5) / (pow(1. + r, 2)));
      ath += temp3 * phinltil;
      aphi += temp4 * phinltil;
    }
...
void SCF_do_center_of_mass_correction(double fac_rad, double start_rad, double fac_part, int max_iter)
{
  ...
  while(iter < max_iter)
      ...
      if(ThisTask == 0)
	{
	  printf("temp. center of mass = (%g|%g|%g)     iter=%d  num_all=%d\n", pos_all[0] / num_all,
		 pos_all[1] / num_all, pos_all[2] / num_all, iter, num_all);
	  printf("done.\n");
	}
  ...
  if(ThisTask == 0)
    {
      printf("SCF center of mass = (%g|%g|%g)   iter=%d\n", pos_all[0] / num_all, pos_all[1] / num_all,
	     pos_all[2] / num_all, iter);
      printf("done.\n");
    }
}
	  ...
	  for(l = 0; l <= SCF_LMAX; l++)
	      ...
	      for(m = 0; m <= l; m++)
		{
		  ...
		  fprintf(FdSCF, "Step %d:   Task: %d   H[%d,%d,%d] = %g %g\n", All.NumCurrentTiStep,
			  ThisTask, n, l, m, sinsum[nlm(n, l, m)],
			  sinsum[nlm(n, l, m)] * scalefac_nlm[nlm(n, l, m)]);
		  fprintf(FdSCF, "Step %d:   Task: %d   G[%d,%d,%d] = %g %g\n", All.NumCurrentTiStep,
			  ThisTask, n, l, m, cossum[nlm(n, l, m)],
			  cossum[nlm(n, l, m)] * scalefac_nlm[nlm(n, l, m)]);
#else
		  fprintf(FdSCF, "Step %d:   Task: %d   H[%d,%d,%d] = %g\n", All.NumCurrentTiStep, ThisTask,
			  n, l, m, sinsum[nlm(n, l, m)]);
		  fprintf(FdSCF, "Step %d:   Task: %d   G[%d,%d,%d] = %g\n", All.NumCurrentTiStep, ThisTask,
			  n, l, m, cossum[nlm(n, l, m)]);
#endif
		  fflush(FdSCF);
		}
	    }
