...
double do_chemcool(int part_index, double dt)
{
  ...
  if(part_index < 0)
      ...
      for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	{
	  if(P[i].Type == 0)
	      ...
	      if(dt > 0)
		{
		  ...
		  yn =
		    All.UnitDensity_in_cgs * HYDROGEN_MASSFRAC * SphP[i].d.Density * a3inv * hubble_param2 /
		    PROTONMASS;

		  energy = energy_old =
		    All.UnitEnergy_in_cgs / pow(All.UnitLength_in_cm,
						3.0) * SphP[i].d.Density * a3inv * hubble_param2 *
		    DMAX(All.MinEgySpec,
			 (SphP[i].Entropy + SphP[i].e.DtEntropy * dt) * pow(SphP[i].d.Density * a3inv,
		  ...
		  entropy_new =
		    energy / All.UnitEnergy_in_cgs * pow(All.UnitLength_in_cm,
							 3.0) / hubble_param2 / pow(SphP[i].d.Density * a3inv,
										    GAMMA) * GAMMA_MINUS1;
  ...
  else
    {
      ...
      yn =
	All.UnitDensity_in_cgs * HYDROGEN_MASSFRAC * SphP[i].d.Density * a3inv * hubble_param2 / PROTONMASS;

      energy = energy_old =
	SphP[i].Entropy * All.UnitEnergy_in_cgs / pow(All.UnitLength_in_cm,
						      3.0) * hubble_param2 * pow(SphP[i].d.Density * a3inv,
										 GAMMA) / GAMMA_MINUS1;
