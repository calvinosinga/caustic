...
void blackhole_accretion(void)
{
  ...
  for(n = FirstActiveParticle; n >= 0; n = NextActiveParticle[n])
	...
	if(norm > 0)
	{
	  ...
	  if (rho > All.PhysDensThresh) AccretionFactor = pow(rho/All.PhysDensThresh, All.BlackHoleAccretionFactor);
	  else AccretionFactor = 1.0;
			  ...
			  if(P[j].Type == 0)
			      ...
			      if(P[j].Mass > 0)
				{
				  ...
				  if(temp > LT_MAX_TEMP_FEEDBACK)
				    {
				      printf("WARNING: task=%d: id=%llu, m=%e large Injected_BH_Energy in blackhole: %e+%e would be temp=%e\n",
					     ThisTask,(unsigned long long) P[j].ID,P[j].Mass,SphP[j].i.dInjected_BH_Energy,energy,temp);
