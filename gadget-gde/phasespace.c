...
void do_phase_space_drift(int i, double dt_drift)
{
  ...
  dt_rate = dt_drift / hubble_function(All.Time);
#endif
  
  
  /* CAUSTIC IDENTIFICATION PART */
  /* projection phase-space distortion matrix (see Vogelsberger et al, 2008 Eq. (22)) -> needed to check for caustic in drift */
  ...
  analyse_phase_space(i, &s_1, &s_2, &s_3, &smear_x, &smear_y, &smear_z, &second_deriv, &sigma);
  ...
  if((determinant * P[i].last_stream_determinant < 0.0) && (((1 << P[i].Type) & (GDE_TYPES))))
    {
...
#endif
    }			/* end if((determinant * P[i].last_stream_determinant < 0.0) && (((1 << P[i].Type) & (GDE_TYPES)))) */
  ...
  /* integrate normed annihilation radiation -> multiplication with initial stream density done in io.c */
...
void analyse_phase_space(MyIDType pindex,
			 ...
			 MyDouble * second_deriv, MyDouble * sigma)
{
  /* for half kick correction of phase-space distortion tensor */
  MyDouble half_kick_add[6][6];
